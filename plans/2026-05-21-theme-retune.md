...
4. **Write `scripts/verify-palette.mjs`** that loads each theme, computes CIE L* for the headline syntax roles, computes WCAG contrast against `#0C0A09`, and asserts: (a) Keywords/Functions/Types/etc. each within ±2 of their claimed L*; (b) no syntax-role hex falls in the 450-520nm wavelength band; (c) the three variants share key sets. Run it and fix any drift it surfaces.
